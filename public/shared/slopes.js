// Slope data + generator — dependency-free DATA (no THREE), an ES module so the
// builder, the gallery, and (via dynamic import) the Node tests can read it without
// the render pipeline. A slope `def` is `{ pieces, ramps, obstacles, width, … }`;
// SlopeBuilder (browser) turns `pieces` into a descending centerline and resolves
// `ramps`/`obstacles` (placed as a fraction `at` of the total run length) into
// arclength `s`.
//
// piece kinds:
...
//
// LIVE PLAY IS FULLY PROCEDURAL — there is no hand-authored slope catalog. Every
// run is `generateSlope(seed, { level })` (below): a fresh random def of the shape
// documented above, tuned by difficulty tier (Blue/Red/Black). (The old hand-authored
// courses — `powder-bowl` plus the `trick-lab`/`bump-lab` practice runs — were retired
// once tracks went fully dynamic.)

// ---- Procedural generator -------------------------------------------------
// A seeded PRNG + slope synth so every run can be a fresh, believable hill. Emits
// the `{ pieces, ramps, obstacles, … }` def shape documented above, so SlopeBuilder
// / SceneRenderer / SkiEngine consume it unchanged — and because
...
// generateSlope(seed, { level, targetLength }) → a slope `def` ({ pieces, ramps,
// obstacles, width, … }). Bounded & fair: a believable descent — monotonic pitch in a
