...
//     counter j = (j+1)&255. The DISPLAY decides how big the pop is (a small hop on
//     open snow, a big launch if the skier is AT a ramp lip); the controller just
...
//
// Contract: exposes get state() → { t: 0|1, j: 0..255 } for the CONTROL tick, plus
// onTuckStart / onTuckEnd callbacks for the HUD + haptics.
...
export class SwipeInput {
  ...
  _up(e) {
    if (e.pointerId !== this._pointerId) return;
    if (this._tucking) {
      // Release of a held tuck = crouch-release JUMP.
      this._endTuck(true);
    } else {
  ...
  _bindKeys() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (k === 'arrowdown' || k === 's') {
        if (!this._keyTuck) {
          this._keyTuck = true; this._tucking = true; this.onTuckStart();
        }
        e.preventDefault();
      } else if (k === 'arrowup' || k === ' ') {
