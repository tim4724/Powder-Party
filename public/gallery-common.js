...
var Gallery = (function() {
  // Skier livery palette names, indexed to match protocol.js SKIER_COLORS.
  ...
  var STATE_KEY = 'powder_gallery_state_v1';
  var defaults = {
    displayAR: '16x9',
  ...
  function _drain() {
    while (!paused && active < MAX_CONCURRENT && queue.length) {
      ...
      // Fallback: the display page loads a WebGL scene, so give it a generous
      // window before assuming the load event was missed. Cleared in cleanup()
      // so it doesn't sit pending for 12s after a normal load.
  ...
  function makeCard(opts) {
    // opts: { title, tag, frameClass, logical, url, chromePx }
